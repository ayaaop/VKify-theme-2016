...
class CMessageBox {
    constructor(options = {}) {
        const title = options.title !== undefined && options.title !== null ? options.title : 'Untitled';
        const body  = options.body !== undefined && options.body !== null ? options.body : '<hr>';
        const buttons = options.buttons !== undefined && options.buttons !== null ? options.buttons : [];
        const callbacks = options.callbacks !== undefined && options.callbacks !== null ? options.callbacks : [];
        const close_on_buttons = options.close_on_buttons !== undefined && options.close_on_buttons !== null ? options.close_on_buttons : true;
        const unique_name = options.unique_name !== undefined && options.unique_name !== null ? options.unique_name : null;
        const warn_on_exit = options.warn_on_exit !== undefined && options.warn_on_exit !== null ? options.warn_on_exit : false;
        const custom_template = options.custom_template !== undefined && options.custom_template !== null ? options.custom_template : null;
    ...
    __exitDialog() {
        ...
        window.messagebox_stack.splice(index_of_item, 1)
    }
...
window.MessageBox = function MessageBox(title, body, buttons, callbacks, return_msg = false) {
    const msg = new CMessageBox({
        title: title,
        body: body,
        buttons: buttons,
